...
1. The `ClusterSummary` reconciler deploys each declared feature, including Helm charts, raw YAML, and Kustomize packages, in the order defined by the profile. A failure on one feature stops further features for that cluster until the next reconciliation cycle.
...
|---|---|---|---|---|
| **access-manager** | `accessManager.enabled` | `true` | 1. Processes `RoleRequest` resources — grants permissions to tenant admins inside managed clusters. 2. Handles `AccessRequest` resources and generates short-lived kubeconfigs for in-cluster agents to call back to the management cluster. | Safe to disable when not using multi-tenancy (`RoleRequest`). |
