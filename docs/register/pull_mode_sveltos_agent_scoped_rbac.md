...
[Restricting sveltos-applier to Specific Namespaces](register_cluster_pull_mode.md#restricting-sveltos-applier-to-specific-namespaces) only covers half of a Pull Mode managed cluster's Sveltos footprint. `sveltos-agent`, the component that evaluates Classifiers, HealthChecks, EventSources and Reloaders, runs in the same cluster and, by default, is also granted a cluster-wide `ClusterRole`: a wildcard `get`/`list`/`watch` grant across every group and resource, so it can evaluate whatever `resourceSelectors` any Classifier/HealthCheck/EventSource happens to target. Restricting `sveltos-applier` alone leaves that second, broader credential untouched.
