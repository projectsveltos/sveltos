...
This demo will showcase Sveltos' capabilities by deploying a Postgres cluster for each managed cluster with label `postgres=required`.
...
Here we created yet another Civo cluster and registered with Sveltos:
...
1. Sveltos will deploy a new Postgres DB in the `services` cluster;
2. Fetch the credentials and external-ip:port info to access the cluster;
3. Deploy a Job in the `pepsi` cluster that creates a table in the DB.