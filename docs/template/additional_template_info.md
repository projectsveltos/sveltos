...
Sveltos automatically fetches these resources, so there's no need to add them to the `TemplateResourceRefs` section. They are readily available for use in the templates (via the specified alias).
