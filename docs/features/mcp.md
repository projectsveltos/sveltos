...
- **Trace Event-Driven Deployments**: The AI can follow an event-driven deployment end-to-end — from EventSource detection on the managed cluster, through EventTrigger evaluation, to the creation and deployment of the resulting dynamic ClusterProfile — and identify exactly where the chain breaks, including surfacing the specific error when event-manager fails to instantiate a ClusterProfile from an EventReport, instead of leaving the user to infer it from a missing profile. It can also list all ClusterProfiles dynamically created by EventTriggers and map each back to its originating event.

- **Trace Classifier Pipelines**: The AI can trace either a `Classifier` or a `ManagementClusterClassifier` instance against a specific cluster by name alone, without needing to know which kind it is. For a `Classifier`, it reports whether the cluster currently matches, the agent report delivery phase, deployment status, and label ownership/conflicts. For a `ManagementClusterClassifier` (which evaluates resources on the management cluster itself rather than deploying anything to managed clusters), it reports cluster match, any reconcile failure (invalid Lua, resource collection errors), and the same label ownership/conflict detail. A separate fleet-wide tool lists every label currently being managed by any `Classifier` or `ManagementClusterClassifier` across all, or a filtered subset of, managed clusters, along with which instance owns each label and where two instances are conflicting over the same key, mirroring `sveltosctl show classifier-labels`.
